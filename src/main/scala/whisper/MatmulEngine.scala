...
/** One matmul / conv / attention-matmul job. Addresses: activation in 256-bit words, weights in
  * 2048-bit words (tile-row groups), params in 1024-bit words, output in 256-bit words. */
class MatmulCmd extends Bundle {
  val rows      = UInt(13.W)   // M >= 1 (rows of this chunk; the sequencer chunks M > accRows)
  val kTiles    = UInt(7.W)    // KT >= 1
  val nTiles    = UInt(12.W)   // NT >= 1
  // activation source
  ...
  val actStride = UInt(8.W)    // 256-bit words per row (or per frame for im2col)
  val actKOff   = UInt(8.W)    // k-tile offset (head select for attention)
  val actUnsigned = Bool()     // activations are uint8 (P.V passes)
  val im2col    = Bool()       // conv: row m, k-tile kt -> frame stride*m + tap - 1, tile kt % tilesPerFrame
  val tilesPerFrame = UInt(6.W)
  val stride2   = Bool()
  val frames    = UInt(13.W)   // valid input frames (rows outside [0, frames) read as zero)
  // weight source
  val wSrc      = UInt(2.W)    // 0 = weight store, 1 = K cache, 2 = V cache
  val wBase     = UInt(20.W)
  val wStrideN  = UInt(20.W)
  val wStrideK  = UInt(12.W)
  val wStrideG  = UInt(12.W)
  // output
  val outMode   = UInt(2.W)    // 0 int8, 1 int16, 2 raw int32 (attention), 3 wide (LM head)
  val outSink   = UInt(2.W)    // 0 activation bank, 1 attention unit, 2 sampler
  val outBank   = UInt(3.W)
  val outBase   = UInt(20.W)
  val outStride = UInt(8.W)    // 256-bit words per row
  ...
  val rowOff    = UInt(13.W)   // row chunk offset: rows [rowOff, rowOff+rows) of the tensor (addresses, im2col, rowfac)
...
object MatmulMode {
  val Int8 = 0; val Int16 = 1; val Raw = 2; val Wide = 3
}

class MatmulOut(cfg: WhisperConfig) extends Bundle {
  val sink   = UInt(2.W)
  val mode   = UInt(2.W)
  val bank   = UInt(3.W)
...
class SystolicArray(cfg: WhisperConfig) extends Module {
  ...
  // register views with reset-initialised valid bits
...
class MatmulEngine(cfg: WhisperConfig) extends Module {
  ...
  val io = IO(new Bundle {
    ...
    val w = new Bundle {
      val src = Output(UInt(2.W)); val addr = Output(UInt(20.W)); val en = Output(Bool())
  ...
  // ------------------------------------------------------------------ load generator
  val ldN = Reg(UInt(12.W)); val ldK = Reg(UInt(7.W)); val ldG = Reg(UInt(2.W))
  val ldAddrN = Reg(UInt(20.W)); val ldAddrK = Reg(UInt(20.W))
  ...
  val acN = Reg(UInt(12.W)); val acK = Reg(UInt(7.W)); val acM = Reg(UInt(13.W))
  val acTilesStarted = Reg(UInt(20.W))
  val acDone = Reg(Bool())
  val acTap = Reg(UInt(2.W)); val acKin = Reg(UInt(6.W))   // im2col tap / k-tile within frame
  val acRowAddr = Reg(UInt(20.W))                          // actBase + m*stride (non-im2col)
  val acFrame0 = Reg(SInt(15.W))                           // stride*m - 1 (frame of tap 0)

  val nTilesTotal = cmd.nTiles * cmd.kTiles
  val frame0Init = (Mux(cmd.stride2, cmd.rowOff << 1, cmd.rowOff).zext - 1.S)(14, 0).asSInt
  ...
  // activation address
  val frame = acFrame0 + acTap.zext
  val frameValid = frame >= 0.S && frame < cmd.frames.zext
  val actAddr = Mux(cmd.im2col,
    (cmd.actBase.zext + frame * cmd.actStride.zext).asUInt + acKin,
    acRowAddr + cmd.actKOff + acK)
  val acFire = canAct
  io.act.en := acFire && (!cmd.im2col || frameValid)
  io.act.src := cmd.actSrc
  io.act.bank := cmd.actBank
  io.act.addr := actAddr(19, 0)
  val acFireQ = RegNext(acFire, false.B)
  val acZeroQ = RegNext(cmd.im2col && !frameValid)
  ...
  actBeat.data := Mux(acZeroQ, 0.U, io.act.data)
  actBeat.tag := acTagQ.asUInt

  when(acFire) {
    when(acM === cmd.rows - 1.U) {
      acM := 0.U; acRowAddr := cmd.actBase + cmd.rowOff * cmd.actStride; acFrame0 := frame0Init
      acTilesStarted := acTilesStarted + 1.U
      // next k-tile
      when(acK === cmd.kTiles - 1.U) {
        acK := 0.U; acTap := 0.U; acKin := 0.U
        when(acN === cmd.nTiles - 1.U) { acDone := true.B } .otherwise { acN := acN + 1.U }
      } .otherwise {
        acK := acK + 1.U
        when(acKin === cmd.tilesPerFrame - 1.U) { acKin := 0.U; acTap := acTap + 1.U } .otherwise { acKin := acKin + 1.U }
      }
    } .otherwise {
      acM := acM + 1.U
      acRowAddr := acRowAddr + cmd.actStride
      acFrame0 := acFrame0 + Mux(cmd.stride2, 2.S, 1.S)
    }
  }
  ...
  // tags travel inside the beats; peek ahead in the act chain for memory reads.
  val outBeat = arr.io.outTag
  val outTagB = outBeat.tag.asTypeOf(new OutTag)
  // acc read: issue when the beat is at row R-2 (data arrives when it is at the bottom + 1 = outBeat)
  ...
  // rowfac & params: issue reads at row R-4 so values are ready at the bottom with margin
  ...
  // data valid one cycle later (beat at R-3); delay to align with outBeat (beat at bottom+1 = R-1+1): 3 more regs
  val rowfacQ = ShiftRegister(Mux(cmd.dynamic, io.rowfac.data, 1.U(16.W)), 3)
  ...
  val rowfacA = RegNext(rowfacQ)
  val biasA = RegNext(biasQ)
  for (n <- 0 until C) prodA(n) := sum(n) * multQ(32 * n + 31, 32 * n).asSInt
  // stage B: t = sat40(rsr(prod, s1))
  val vB = RegNext(vA, false.B); val tagB = RegNext(tagA); val rowfacB = RegNext(rowfacA); val biasB = RegNext(biasA)
  val tB = Reg(Vec(C, SInt(40.W)))
  for (n <- 0 until C) {
    val rnd = Mux(cmd.s1 === 0.U, prodA(n), ((prodA(n) + (1.S(64.W) << (cmd.s1 - 1.U))) >> cmd.s1))
    tB(n) := Sat.sint(rnd, 40)
  }
  // stage C: u = t*rowfac + (bias << (s2-16))
  val vC = RegNext(vB, false.B); val tagC = RegNext(tagB)
  val uC = Reg(Vec(C, SInt(64.W)))
  val tC = RegNext(tB)
  for (n <- 0 until C) {
    val b = biasB(32 * n + 31, 32 * n).asSInt
    uC(n) := tB(n) * Cat(0.U(1.W), rowfacB).asSInt + (b << (s2 - 16.U))
  }
  ...
  when(io.cmd.fire) {
    ...
    acTap := 0.U; acKin := 0.U; acRowAddr := io.cmd.bits.actBase + io.cmd.bits.rowOff * io.cmd.bits.actStride
    acFrame0 := (Mux(io.cmd.bits.stride2, io.cmd.bits.rowOff << 1, io.cmd.bits.rowOff).zext - 1.S)(14, 0).asSInt
    cycles := 0.U
  }

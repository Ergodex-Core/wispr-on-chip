...
object VerilatorSettings {
  def threads: Int = sys.env.get("WHISPER_SIM_THREADS").map(_.toInt).getOrElse(1)
  def fast: Boolean = sys.env.get("WHISPER_SIM_FAST").forall(_ != "0")
...
trait WhisperSim extends chisel3.simulator.scalatest.ChiselSim { self: TestSuite =>
  implicit val whisperHasSimulator: HasSimulator =
  ...
  implicit val whisperFirtoolOpts: FirtoolOptionsModifications =

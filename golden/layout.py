"""Memory-layout helpers shared by the vector generators and the RTL tests (docs/tiling.md).
...
def act32_words(x32: np.ndarray, stride_words: int | None = None) -> np.ndarray:
    """int32 [M, N] -> uint32 words, 4 words per 32 columns (8 lanes per 256-bit word)."""
    x32 = np.asarray(x32, dtype="<i4")
    M, N = x32.shape
    assert N % 32 == 0
    sw = stride_words or N // 8
    buf = np.zeros((M, sw * 8), dtype="<i4")
    buf[:, :N] = x32
    return buf.reshape(-1).view("<u4")


def words_from_act32(words: np.ndarray, M: int, N: int, stride_words: int) -> np.ndarray:
    b = np.asarray(words, dtype="<u4").view(np.uint8).reshape(M, stride_words * 32)
    return b[:, : 4 * N].view("<i4")
...
def write_bin(path, words: np.ndarray):
    np.ascontiguousarray(np.asarray(words, dtype="<u4")).tofile(path)

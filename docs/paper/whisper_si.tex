...
\item Measured performance (Section~\ref{sec:perf}): cycle counts, an analytic model that reproduces them, and ASAP7 \SI{7}{nm} synthesis of every compute block against a \SI{1}{GHz} target, including the blocks that miss it and what they would cost to fix.
\end{itemize}
...
\paragraph{Latency and throughput} Cycle counts are exact, so latency is one division away once a clock is fixed. Table~\ref{tab:latency} gives the three that matter: the design as synthesised (\fmaxAll, attention-limited), the clock it reaches once the two chains of Section~\ref{sec:synth} are pipelined (\SI{0.75}{GHz}, then engine-limited), and the \SI{1}{GHz} target. At \SI{1}{GHz} a \SI{30}{s} window costs \SI{42.1}{ms} plus \SI{0.2}{ms} per generated token, so a typical \SI{10}{s} utterance of 30 tokens is transcribed in \SI{48}{ms}, a real-time factor of $5\times10^{-3}$, and one engine sustains about 17 windows per second, or \SI{500}{s} of audio per second of wall time. Because the encoder runs for any $n_{\mathrm{frames}}\le3000$, a host that pads to the true speech length rather than \SI{30}{s} would cut the encoder cost roughly in proportion, at the accuracy cost documented in decision~12.

\begin{table}[t]
\caption{Latency from the measured cycle counts at three clocks. Only the first column is a synthesised result; the others state what the same cycle counts buy at a higher clock.}
\label{tab:latency}
\centering\small
\begin{tabular}{@{}lrrr@{}}
\toprule
 & as synthesised & chains pipelined & target \\
 & \fmaxAll & \SI{0.75}{GHz} & \SI{1}{GHz} \\
\midrule
\SI{30}{s} window (42.1\,M cycles) & \SI{248}{ms} & \SI{56}{ms} & \SI{42}{ms} \\
per generated token (200\,k) & \SI{1.2}{ms} & \SI{0.27}{ms} & \SI{0.20}{ms} \\
\SI{10}{s} utterance, 30 tokens & \SI{283}{ms} & \SI{64}{ms} & \SI{48}{ms} \\
real-time factor & $9{\times}10^{-3}$ & $2{\times}10^{-3}$ & $2{\times}10^{-3}$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Power} We have no switching-activity simulation, so we give only a first-order bound. With the array \SI{70}{\percent} busy and \SI{50}{fJ} per int8 MAC including its 32-bit accumulate (a mid-range figure for \SI{7}{nm} int8 arrays), the 1024 MACs draw about \SI{36}{mW} at \SI{1}{GHz} (the clock the design would reach after the pipelining above, not as synthesised); a 2048-bit weight-tile word every four cycles and a 256-bit activation word per cycle at \SI{20}{fJ\per bit} add about \SI{15}{mW}, and the remaining logic and clock tree perhaps as much again. A total of order \SI{100}{mW} at \SI{1}{GHz}, i.e.\ about \SI{5}{mJ} per \SI{30}{s} window, is our estimate; it is not a measurement.
...
Accuracy runs use the full \SI{30}{s} window rather than a speech-length context; calibration used dev-clean only, and the requant saturation counter is the only runtime guard on unseen inputs; the two end-to-end sets overlap in ten utterances, so 24 distinct clips ran on the RTL. The synthesis numbers are pre-layout, come from an open-source mapper rather than a signoff tool, and exclude the memories, whose area is estimated from published densities; no place-and-route or power simulation was run, and wire load is not modelled, so the reported paths are optimistic. Reaching \SI{1}{GHz} needs three pipeline changes we did not make here: the attention softmax step, the LayerNorm Newton chain, and the matmul drain's requant multiply. The performance steps beyond that are a second softmax pipeline overlapping the $PV$ of one tile with the exponentials of the next, and letting the vector unit run concurrently with the engine on independent rows; the analytic model sizes all of them before they are built.
